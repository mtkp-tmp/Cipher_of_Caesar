print(''.join((lambda sh, text : [''.join((lambda sh, text : [''.join((lambda sh, text :
['abcdefghijklmnopqrstuvwxyz'[(i + sh % 26)] for i in map('abcdefghijklmnopqrstuvwxyz'.index, text)])(sh, l)) if ord(l) in range(97,123) else
''.join((lambda sh, text : ['ABCDEFGHIJKLMNOPQRSTUVWXYZ'[(i + sh) % 26] for i in map('ABCDEFGHIJKLMNOPQRSTUVWXYZ'.index, text)])(sh, l)) for l in text])(sh, l)) 
...
''.join((lambda sh, text : ['АБВГДЕЁЖЗИЙКЛМНОПРСТУФХЦЧШЩЪЫЬЭЮЯ'[(i + sh) % 33] for i in map('АБВГДЕЁЖЗИЙКЛМНОПРСТУФХЦЧШЩЪЫЬЭЮЯ'.index, text)])(sh, l)) for l in text])(sh,l))
